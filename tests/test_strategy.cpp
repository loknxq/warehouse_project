// warehouse_project/tests/test_strategy.cpp
...
TEST_CASE("AverageSalesStrategy tests", "[Strategy]") {
    ...
    SECTION("calculate handles edge cases") {
        ...
        // Negative values - should treat as zero
        // currentStock = -5 -> treated as 0, deficit = 10 - 0 = 10, result = 10 + 6 = 16
        REQUIRE(strategy.calculate(-5, 10, 2, 3) == 16);
        
        // All negative values
        REQUIRE(strategy.calculate(-10, -5, -2, -3) == 0); // minStock treated as 0, current treated as 0
    }
